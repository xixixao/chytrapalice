...
class DataGrid extends Control implements ArrayAccess, INamingContainer
{
	...
	/** @var array */
	public $displayedItems = array('vše', 5, 10, 15, 20, 50, 100);
  ...
  public $hiddenFiltering = FALSE;

	/** @var string */
	public $defaultFilters;
	
	/** @var string */
	public $nothingMessage = "Nic nebylo nalezeno.";
    ...
    setcookie("itemsPerPage",$value, time()+604800);
	...
	protected function applyPaging()
	{
		...
		if ($this->wasRendered && $this->paginator->itemCount < 1 && !empty($this->filters)) {
			...
			$this->getTemplate()->flashes[] = (object) array(
				'message' => $this->translate("Žádný záznam neodpovídá vyhledávání."),
				'type' => 'info',
			);
		}
	...
	public function render()
	{
		if (!$this->wasRendered) {
			...
			if (!count($this->dataSource)) {
				$this->flashMessage($this->translate($this->nothingMessage), 'info');
			}
	...
	protected function createComponentForm($name)
	{
		...
		$form->addSubmit('resetSubmit', 'Resetovat');
		$form->addSubmit('filterSubmit', 'Filtrovat');

		$form->addSelect('operations', 'Vybrané:', $this->operations);
		$form->addSubmit('operationSubmit', 'Poslat')->onClick = $this->onOperationSubmit;
		...
		$form->addSelect('items', 'Položek na stránce', array_combine($this->displayedItems, $this->displayedItems));
